...
export function SecurityDashboardPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex h-64 items-center justify-center" aria-busy="true" role="status">
        <span className="text-xl font-bold dark:text-white">Loading Security Dashboard...</span>
      </div>
    );
  }
  ...
  const depStats = dependencies || { total_outdated: 0, avg_decay_rate: 0, avg_security_score: 100 };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 pt-24 pb-12 space-y-10">
      <section className="rounded-[2rem] border-4 border-black bg-indigo-600 p-8 sm:p-10 shadow-card relative overflow-hidden dark:border-[#2e2924] dark:shadow-none">
        <div className="relative z-10 text-white">
          ...
          <p className="text-lg font-bold bg-white/10 p-4 rounded-lg border-4 border-white shadow-card-sm inline-block max-w-lg leading-relaxed backdrop-blur-sm">
            Monitor software rot, track dependency aging, and review automatically generated Pull Requests for updates.
          </p>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="rounded-[2rem] border-4 border-black bg-white p-6 shadow-card flex flex-col justify-between dark:bg-[#1f1c18]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xs uppercase tracking-widest text-gray-500">Global Score</h3>
              <p className="text-4xl font-black text-black dark:text-white">{depStats.avg_security_score.toFixed(0)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-[2rem] border-4 border-black bg-white p-6 shadow-card flex flex-col justify-between dark:bg-[#1f1c18]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xs uppercase tracking-widest text-gray-500">Outdated Packages</h3>
              <p className="text-4xl font-black text-black dark:text-white">{depStats.total_outdated}</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-[2rem] border-4 border-black bg-white p-6 shadow-card flex flex-col justify-between dark:bg-[#1f1c18]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xs uppercase tracking-widest text-gray-500">Critical CVEs</h3>
              <p className="text-4xl font-black text-black dark:text-white">{metrics?.critical || 0}</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-[2rem] border-4 border-black bg-white p-6 shadow-card flex flex-col justify-between dark:bg-[#1f1c18]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xs uppercase tracking-widest text-gray-500">Active Auto-PRs</h3>
              <p className="text-4xl font-black text-black dark:text-white">{metrics?.active_autofix_prs || 0}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-[2rem] border-4 border-black bg-white p-6 shadow-card dark:bg-[#1f1c18] dark:border-[#2e2924]">
        <h2 className="text-2xl font-black mb-6 text-black dark:text-white">Dependency Decay Rate</h2>
        <div className="h-24 w-full flex items-center">
          <div className="w-full bg-gray-200 rounded-full h-8 overflow-hidden border-2 border-black flex">
            <div 
              ...
              style={{ width: `${Math.max(0, 100 - depStats.avg_decay_rate * 100)}%` }}
            >
              Fresh
            </div>
            <div 
              className="bg-red-500 h-full flex items-center justify-center text-xs font-bold text-white"
              style={{ width: `${Math.min(100, depStats.avg_decay_rate * 100)}%` }}
            >
              Decaying
            </div>
          </div>
